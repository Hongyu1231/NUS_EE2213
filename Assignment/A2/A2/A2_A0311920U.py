...
def A2_A0311920U(road_map, city_coordinates, start_city, destination_city):
    """
    Input types:
    :road_map type: dict
    ...
    # ...
    node_data = {city : {'dist' : math.inf, 'prev' : []} for city in road_map}
    visited = set()
    queue = []
    heappush(queue, (0, start_city))
    node_data[start_city]['dist'] = 0   

    # Main A* search loop goes here
    # ...
    while queue:
        dist, city = heappop(queue)
        if city == destination_city:
            path = node_data[city]['prev'] + [city]
            return path, node_data[city]['dist']
        else:
            visited.add(city)
            
        for neighbor in road_map[city].keys():
            if neighbor not in visited:
                g = road_map[city][neighbor] + node_data[city]['dist']
                if g < node_data[neighbor]['dist']:
                    node_data[neighbor]['dist'] = g
                    node_data[neighbor]['prev'] = node_data[city]['prev'] + [city]
                    heappush(queue, (g + heuristic(neighbor, destination_city), neighbor))
